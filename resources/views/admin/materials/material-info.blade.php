...
<body>
    ...
    <div class="container mt-5 mb-5">
        <div class="row">
            <div class="col-md-12">
                <h2 class="text-center mb-4">ข้อมูลวัสดุ</h2>
                ...
                <form method="GET" action="{{ route('admin.materials.index') }}"
                    ...
                    <div class="input-group shadow-sm" style="max-width: 500px; width: 100%;">
                        ...
                        <input type="text" name="keyword" class="form-control border-start-0"
                            placeholder="ค้นหาวัสดุ..." value="{{ request('keyword') }}">
                        @if(request('keyword'))
                            <a href="{{ route('admin.materials.index') }}" class="btn btn-outline-secondary">ล้าง</a>
                        @endif
                    </div>
                </form>
            </div>
            ...
            <div class="d-flex flex-wrap justify-content-center gap-2 mb-4">
                ...
                <a href="{{ route('admin.materials.index', ['status' => 'พร้อมใช้งาน']) }}"
                    class="btn {{ request('status') == 'พร้อมใช้งาน' ? 'btn-success' : 'btn-outline-success' }}">
                    พร้อมใช้งาน ({{ $statusSummary['พร้อมใช้งาน'] ?? 0 }})
                </a>
                <a href="{{ route('admin.materials.index', ['status' => 'หมดคลัง']) }}"
                    class="btn {{ request('status') == 'หมดคลัง' ? 'btn-warning text-dark' : 'btn-outline-warning' }}">
                    หมดคลัง ({{ $statusSummary['หมดคลัง'] ?? 0 }})
                </a>
                <a href="{{ route('admin.materials.index', ['status' => 'ชำรุด']) }}"
                ...
                <table class="table table-bordered table-striped table-hover">
                    ...
                    <tbody>
                        @foreach($materials as $material)
                            <tr>
                                <td><input type="checkbox" class="selectItem" value="{{ $material->id }}"></td>
                                <td class="text-center">
                                    {{ ($materials->currentPage() - 1) * $materials->perPage() + $loop->iteration }}
                                </td>
                                <td class="text-center">{{ $material->material_code }}</td>
                                <td>{{ $material->name }}</td>
                                <td class="text-center">{{ $material->category->name ?? 'ไม่พบหมวดหมู่' }}</td>
                                ...
                                <td class="text-center">
                                    @if($material->image)
                                        <img src="{{ asset('storage/' . $material->image) }}" alt="รูปภาพวัสดุ" width="100">
                                    @else
                                ...
                                <td class="text-center">
                                    <a href="{{ route('admin.materials.edit', $material->id) }}"
                                        class="btn btn-warning btn-sm se-1">
                                        <i class="bi bi-pencil"></i> แก้ไข / เพิ่มจำนวน
                                    </a>
            ...
            @if ($materials->hasPages())
                <nav class="d-flex justify-content-center mt-4">
                    <ul class="pagination">
                        ...
                        @else
                            <li class="page-item">
                                <a class="page-link" href="{{ $materials->previousPageUrl() }}" rel="prev">
                                    <i class="bi bi-arrow-left"></i> ก่อนหน้า
                                </a>
                            </li>
                        @endif
                        ...
                        @if ($materials->hasMorePages())
                            <li class="page-item">
                                <a class="page-link" href="{{ $materials->nextPageUrl() }}" rel="next">
    ...
    <!-- JS: การจัดการ checkbox และลบหลายรายการ -->
    <script>
            ...
            deleteSelectedButton.addEventListener('click', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        fetch('{{ route('admin.materials.destroySelected') }}', {
                            method: 'POST',
                            headers: {
    ...
    <!-- 🚨 SweetAlert แจ้งเตือนวัสดุหมดคลัง -->
@if (!empty($lowStockMaterials))
    ...
    <script>
        Swal.fire({
            icon: 'warning',
            title: 'วัสดุหมดคลัง!',
            html: `{!! implode('<br>', $lowStockMaterials) !!}`,
