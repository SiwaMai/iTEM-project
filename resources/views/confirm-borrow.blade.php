...
<body>
    ...
    <div class="container mt-5">
        <h3>ยืนยันการยืมครุภัณฑ์</h3>

        <div class="card mb-3">
            <div class="card-body">
                <strong>ชื่อครุภัณฑ์:</strong> {{ $equipment->name }}<br>
                <strong>สถานะ:</strong> {{ $equipment->status }}<br>
                <strong>จำนวนคงเหลือ:</strong> {{ $equipment->quantity }}
            </div>
        </div>
        ...
        <form id="borrowForm">
            ...
            <input type="hidden" id="unit" value="{{ $equipment->unit ?? 'ชิ้น' }}">

            <div class="mb-3">
                <label for="quantity" class="form-label">จำนวนที่ต้องการยืม:</label>
                <input type="number" id="quantity" name="quantity" class="form-control" min="1"
                    max="{{ $equipment->quantity }}" required>
            </div>

            <div class="mb-3">
                <label for="borrowed_at" class="form-label">วันที่ยืม:</label>
                <input type="date" id="borrowed_at" name="borrowed_at" class="form-control" value="{{ date('Y-m-d') }}"
                    required>
            </div>

            <div class="mb-3">
                <label for="due_date" class="form-label">วันที่ต้องคืน:</label>
                <input type="date" id="due_date" name="due_date" class="form-control" required>
            </div>

            <div class="mb-3">
                <label for="reason" class="form-label">เหตุผลในการยืม:</label>
        ...
        <div id="borrow-status" class="mt-3"></div>
    </div>
...
<script>
    document.getElementById('borrowForm').addEventListener('submit', function (e) {
        ...
        fetch(`/borrow/submit/${equipmentId}`, {
            method: 'POST',
            headers: {
        ...
        .then(async response => {
            ...
            if (!response.ok) {
                if (contentType && contentType.includes("application/json")) {
                    ...
                    throw new Error(errorData.message || 'ไม่สามารถส่งคำขอยืมได้');
                } else {
        ...
        .then(data => {
            const equipmentName = data.data?.equipment_name || '{{ $equipment->name }}';
            ...
            Swal.fire({
                ...
                html: `
                    <p>อุปกรณ์: <strong>${equipmentName}</strong></p>
                    <p>จำนวน: ${quantity} ${unit}</p>
                    <p>เหตุผล: ${reason}</p>
                    <p>สถานะ: ${status}</p>
                `,
                icon: 'success',
                ...
                window.location.href = "{{ route('borrow.list') }}";
            });
        })
        .catch(error => {
            Swal.fire({
